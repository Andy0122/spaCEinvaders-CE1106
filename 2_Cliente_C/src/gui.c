...
void inicializar_gui() {
    ...
    ImageResize(&img_bunker, 60, 40); // Dimensiones estándar del escudo
...
void dibujar_matriz_aliens(Extraterrestre aliens[], int total_aliens) {
    for (int i = 0; i < total_aliens; i++) {
        if (aliens[i].estado == 1) { 
            if (aliens[i].tipo == 40) {
                DrawTexture(tex_pulpo, aliens[i].x, aliens[i].y, WHITE);
            } 
            else if (aliens[i].tipo == 20) {
                DrawTexture(tex_cangrejo, aliens[i].x, aliens[i].y, WHITE);
            } 
            else if (aliens[i].tipo == 10) {
                DrawTexture(tex_calamar, aliens[i].x, aliens[i].y, WHITE);
            }
        }
    }
}
